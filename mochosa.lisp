...
(defun get-new-res (url new-res-num-str dialog-list vbox1 scrolled window vadj)
  (let ((dialog-n 0)
	      (new-res (tikan-dayo (nth-value 0 (dex:get
					                                 (concatenate 'string url new-res-num-str))))))
    (if (not (equal "" new-res))
	      (let* ((ress (make-res new-res))
	        ...
	        (gtk-widget-modify-font dialog
				                          (pango-font-description-from-string
				                           *font*))
	        ;;(gtk-window-set-keep-above dialog t);;最前面に表示
	        ;;okボタン押したら消える
	        #|
	        (g-signal-connect dialog "response"
			    (lambda (widget response)
			    (declare (ignore widget response))
			    (gtk-widget-destroy dialog)))
	        |#
	        (push dialog dialog-list)
	        (dolist (dia1 (reverse dialog-list))
	          (gtk-window-move dia1 ;;ダイアログ表示する位置
			                       (- (gdk-screen-width) 400)
			                       (* dialog-n 130))
	          (incf dialog-n))
	        (dolist (dia1 dialog-list)
	          (sb-ext:run-program "/usr/bin/paplay" *sound*) ;;音ならす
	          ;;(ccl:run-program "/usr/bin/paplay" *sound*)
	          (gtk-widget-show dia1))
	        ;;popuoが出た時だけ(一度だけ)カウントダウンなのでnilを返す
	        (g-timeout-add *popup-time* (lambda ()
					                              (gtk-widget-destroy (car (last dialog-list)))
					                              (setf dialog-list (delete (car (last dialog-list))
								                                                  dialog-list))
					                              nil))
	        (g-timeout-add 1000 (lambda () ;;スクロールウィンドウの一番下へ
			                          (gtk-adjustment-set-value vadj (- (gtk-adjustment-upper vadj)
				                        ...
				                        nil))
	        (g-signal-connect window "destroy" ;;ダイアログ消す
                            (lambda (w)
                              (declare (ignore w))
			                        (if dialog-list
			                            (dolist (dialog dialog-list)
				                            (gtk-widget-destroy dialog)))))
...
(defun main ()
  (within-main-loop
    (let* ((window (make-instance 'gtk-window
	         ...
	         (new-res-num 0)
	         (new-res-num-str nil) (vadj (gtk-scrolled-window-get-vadjustment scrolled))
           ...
           (title-entry (make-instance 'gtk-entry :text (if (null preurl) "" preurl) :width-request 400))
	         (url nil)
	         (id 0) (dialog-list nil) (load-label-id 0) (auto-time 1)
			       ...
			       (progn
			         ...
			         (gtk-widget-destroy vbox1) ;;なんだ？
				       ...
				       (let ((res-list
				         ...
				         (setf new-res-num-str (write-to-string new-res-num))
				         (dolist (res res-list)
         ...
         (if (gtk-switch-active widget);; t:on nil:off
	           (if url
		             (setf id (g-timeout-add
                           *auto-reload-time*
					                 (lambda ()
					                   (if (get-new-res url new-res-num-str dialog-list vbox1
							                                scrolled
							                                window vadj)
						                     (setf new-res-num (1+ new-res-num)
						                           new-res-num-str (write-to-string new-res-num)))
